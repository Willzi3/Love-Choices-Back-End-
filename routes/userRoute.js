  ...
  //Update users
    router.put("/:id", middleware, (req, res) => {
      ...
      try {
        con.query(
          //When using the ${}, the content of con.query MUST be in the back tick
          `UPDATE users set email="${email}", password="${hash}", full_name="${full_name}", phone="${phone}", user_type="${user_type}", birth_date="${birth_date}", gender="${gender}", marital_status="${marital_status}", occupation="${occupation}", address="${address}", description="${description}", image="${image}" WHERE user_id ="${req.params.id}"`,
          (err, result) => {
            if (err) throw err;
            res.send(result);
          }
